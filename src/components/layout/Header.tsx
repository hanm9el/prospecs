...
const MENU = [
  ...
  },
  {
    ...
    subMenu: [
      ...
      { name: "마라톤 슬램 82", path: "/heritage/grand-slam-82" },
    ],
  },
  {
    name: "SPORTS",
...
function Header() {
  ...
  // 스크롤이 내려갔는지 안내려갔는지를 체크해서 스타일링을 해줘야 함
  const [isScrolled, setIsScrolled] = useState(false);

  useEffect(() => {
    // 사용자가 스크롤을 조금이라도 움직이면 이 handleScroll이라고 하는 함수가 발동되는데
    // 만약 사용자가 Y스크롤의 값을 0보다 크게 가져간다면 (즉, 스크롤을 내렸다면)
    // setIsScrolled의 값을 true로 변환. Y스크롤의 값이 0이 됨다면 false 변환.
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 0);
    };

    // addEventListener(동작방식, 함수);
    // addEventListener : 동작방시게 기재한 것을 감지하여 함수를 실행하는 메소드
